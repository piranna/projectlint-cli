...
const { IndicationBadge: {Fail} } = require('tasktree-cli/lib/Theme')
    ...
    .then(function(evaluated)
    {
      switch(evaluated.level)
      {
        ...
        case 2:          // Failed rules configured with `error` level
        {
          ...
          const {result, stack} = error

          if(!Array.isArray(result))
            taskRule.error(stack || error.toString())
          else
            for(const error of result)
              taskRule.warn(error.toString())

          if(!evaluated.error && evaluated.failure) taskRule.setBadge(Fail)
...
module.exports = function(stdout, stderr)
{
  return {
    ...
    {
      const tree = TaskTree.tree(null, stdout, stderr)
